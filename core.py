"""
Factor Momentum core — tested implementation.

This file is the single source of truth that gets transplanted into the
notebook cells. It contains every fix and refinement:

  * true next-day implementation via `implementation_lag` (default 1)
  * stationary bootstrap run on the COMMON sample (dropna fix)
  * paired difference bootstrap (Mom - benchmark) with P(Mom > bench)
  * Sortino denominator = target semideviation (RMS of downside)
  * fixed-bps transaction-cost mode as a sanity check next to AR/CS
  * SPY market benchmark threaded through realized tables and the bootstrap
  * one-way turnover reported alongside the two-way figure
  * random-selection (signal-permutation) null to isolate the signal
"""
...
def fetch_fred_series(series_id, start, end, _pdr=None):
    if _pdr is None:
        raise ImportError("pandas_datareader required.")
    s = _pdr.DataReader(series_id, "fred", start, end)
...
def build_historical_rf_daily(index, rf_source="DGS3MO", day_count=360, lag_days=1,
                              _fetch=fetch_fred_series, _pdr=None):
    ...
    if rf_source == "SOFR" and idx.min() < pd.Timestamp("2018-04-03"):
        raise ValueError("SOFR starts 2018-04-03; use DGS3MO for a full 2013+ sample.")
    start = idx.min() - pd.Timedelta(days=10)
    end = idx.max() + pd.Timedelta(days=10)
    y = _fetch(rf_source, start, end, _pdr=_pdr)
...
def probabilistic_sharpe_ratio(returns, sr_benchmark=0.0, ann=252):
    ...
    accounting for skewness and kurtosis (Bailey & Lopez de Prado).
    Pass EXCESS returns when the intended statistic is an excess-return Sharpe."""
    from scipy import stats
...
def deflated_sharpe_ratio(returns, sr_trials, ann=252):
    ...
    Sharpe ratios of all specifications tried. `returns` and `sr_trials` must use the
    same raw-versus-excess-return convention.
    """
    from scipy import stats
...
# ============================================================
# THESIS EXTENSIONS: rank/quantile weighting (N-agnostic)
# ============================================================

def w_quantile_long(sig_m, q=0.4):
    """Equal-weight the TOP q-quantile of factors by momentum each month (long-only).
    N-agnostic: for 5 factors q=0.4 -> top 2; for 12 factors -> top 5. Removes the
    arbitrary 80/20 / 35/35 concentration so selection is separated from concentration."""
    n = sig_m.shape[1]
    k = max(1, int(np.ceil(q * n)))
    cols = list(sig_m.columns)
    arr = np.zeros((len(sig_m), n), dtype=float)
    for i, (_, row) in enumerate(sig_m.iterrows()):
        top = row.nlargest(k).index
        for c in top:
            arr[i, cols.index(c)] = 1.0 / k
    return pd.DataFrame(arr, index=sig_m.index, columns=cols)


def w_rank_weighted(sig_m, top_frac=1.0):
    """Long-only weights proportional to the cross-sectional RANK of the signal
    (smooth tilt instead of a hard cut). top_frac<1 keeps only the top fraction."""
    R = sig_m.rank(axis=1, method="average")
    n = sig_m.shape[1]
    if top_frac < 1.0:
        k = max(1, int(round(top_frac * n)))
        keep = R.apply(lambda r: r >= r.nlargest(k).min(), axis=1)
        R = R.where(keep, 0.0)
    w = R.div(R.sum(axis=1), axis=0)
    return w.fillna(0.0)


# ============================================================
# THESIS EXTENSIONS: true walk-forward / out-of-sample selection
# ============================================================

def walk_forward_oos(spec_matrix, ew_returns=None, rf_daily=None,
                     min_train_years=2, select_by="sharpe", ann=252):
    """
    Honest out-of-sample evaluation of the specification SEARCH. Each calendar year's
    returns come from the single spec chosen using ONLY data from prior years
    (expanding window). Chains the per-year OOS segments. This answers the data-snooping
    objection directly (not just inferentially as White/SPA do).
    select_by: 'sharpe' | 'cagr' | 'mean' on the in-sample window.
    """
    M = pd.DataFrame(spec_matrix).dropna(how="any").sort_index()
    years = sorted(set(M.index.year))
    oos_parts, chosen = [], {}
    for y in years:
        train = M.loc[M.index.year < y]
        if train.index.year.nunique() < min_train_years or len(train) < 60:
            continue
        seg = M.loc[M.index.year == y]
        if len(seg) == 0:
            continue
        if select_by == "sharpe":
            score = train.apply(lambda c: _sharpe_ann(c, rf_daily=rf_daily, ann=ann))
        elif select_by == "cagr":
            score = train.apply(lambda c: cagr(c, ann=ann))
        elif select_by == "mean":
            score = train.mean()
        else:
            raise ValueError("select_by must be 'sharpe', 'cagr' or 'mean'")
        best = score.idxmax()
        chosen[y] = best
        oos_parts.append(seg[best].rename("oos"))
    oos = pd.concat(oos_parts) if oos_parts else pd.Series(dtype=float, name="oos")
    chosen = pd.Series(chosen, name="best_spec")
    n_switch = int((chosen.values[1:] != chosen.values[:-1]).sum()) if len(chosen) > 1 else 0
    res = {"oos_returns": oos, "chosen_by_year": chosen, "n_switches": n_switch,
           "oos_metrics": perf_metrics(oos, rf_daily=rf_daily, ann=ann)}
    if ew_returns is not None:
        ew_oos = pd.Series(ew_returns).reindex(oos.index)
        res["ew_oos_metrics"] = perf_metrics(ew_oos, rf_daily=rf_daily, ann=ann)
        res["oos_minus_ew_cagr"] = float(cagr(oos, ann) - cagr(ew_oos, ann))
        res["oos_minus_ew_sharpe"] = float(_sharpe_ann(oos, rf_daily, ann) - _sharpe_ann(ew_oos, rf_daily, ann))
    return res


# ============================================================
# THESIS EXTENSIONS: conditional (regime) spanning + replication
# ============================================================

def spanning_regression_conditional(strat_returns, factor_returns, regime, rf_daily=None,
                                    extra_returns=None, lags=None, ann=252, interact_factors=False):
    """
    Spanning regression with a regime indicator `regime` (1 = regime of interest, e.g.
    high-vol). Models a regime SHIFT in alpha: y = a0 + a1*D + B*factors (+ optional
    factor*D interactions). Reports alpha in each regime with Newey-West inference, to
    test whether the alpha is zero only UNCONDITIONALLY.
    """
    from scipy import stats
    s = pd.Series(strat_returns).dropna()
    F = pd.DataFrame(factor_returns).reindex(s.index)
    if extra_returns is not None:
        ex = extra_returns.to_frame() if isinstance(extra_returns, pd.Series) else pd.DataFrame(extra_returns)
        F = pd.concat([F, ex.reindex(s.index)], axis=1)
    D = pd.Series(regime).reindex(s.index)
    idx = s.index.intersection(F.dropna().index).intersection(D.dropna().index)
    s, F, D = s.loc[idx], F.loc[idx], D.loc[idx].astype(float)
    rf = _align_rf_to_returns(s, rf_daily=rf_daily)
    y = (s - rf).values
    Fe = F.sub(rf, axis=0)
    cols = ["alpha", "alpha_shift_D"] + list(F.columns)
    Xparts = [np.ones(len(y)), D.values] + [Fe[c].values for c in F.columns]
    if interact_factors:
        for c in F.columns:
            Xparts.append((Fe[c] * D).values); cols.append(f"{c}xD")
    X = np.column_stack(Xparts)
    res = newey_west_ols(y, X, lags=lags)
    coefs = pd.DataFrame({"coef": res["beta"], "se": res["se"], "t": res["t"]}, index=cols)
    coefs["pval"] = 2 * stats.norm.sf(np.abs(coefs["t"]))
    a0, a1 = float(res["beta"][0]), float(res["beta"][1])
    return {"alpha_D0_ann": a0 * ann, "alpha_D1_ann": (a0 + a1) * ann,
            "alpha_shift_ann": a1 * ann, "shift_t": float(res["t"][1]),
            "shift_p": float(coefs.loc["alpha_shift_D", "pval"]),
            "r2": float(res["r2"]), "n": int(res["n"]), "nw_lags": int(res["lags"]),
            "coefficients": coefs}


def spanning_replication(strat_returns, factor_returns, rf_daily=None, extra_returns=None,
                         lags=None, ann=252):
    """
    Build the STATIC factor-mimicking portfolio implied by the spanning betas (alpha
    excluded) and compare it to the strategy. High correlation + tiny tracking error +
    near-equal Sharpe makes 'spanned' visual: a fixed factor blend reproduces the strategy.
    """
    sp = spanning_regression(strat_returns, factor_returns, rf_daily=rf_daily,
                             extra_returns=extra_returns, lags=lags, ann=ann)
    betas = sp["coefficients"].drop(index="alpha")["coef"]
    s = pd.Series(strat_returns).dropna()
    F = pd.DataFrame(factor_returns).reindex(s.index)
    if extra_returns is not None:
        ex = extra_returns.to_frame() if isinstance(extra_returns, pd.Series) else pd.DataFrame(extra_returns)
    ...
    s, F = s.loc[idx], F.loc[idx]
    rf = _align_rf_to_returns(s, rf_daily=rf_daily)
    Fe = F.sub(rf, axis=0)[betas.index]
    repl_excess = Fe.values @ betas.values
    repl = pd.Series(repl_excess + rf.values, index=idx, name="static_replication")
    strat_ex = (s - rf).values
    corr = float(np.corrcoef(strat_ex, repl_excess)[0, 1])
    te = float(np.std(strat_ex - repl_excess, ddof=1) * np.sqrt(ann))
    return {"replication_returns": repl, "alpha_annual": sp["alpha_annual"],
            "alpha_t": sp["alpha_t"], "corr": corr, "tracking_error_ann": te,
            "sharpe_strat": _sharpe_ann(s, rf_daily, ann),
            "sharpe_replication": _sharpe_ann(repl, rf_daily, ann), "betas": betas}


# ============================================================
# THESIS EXTENSIONS: mechanism of the positive finding (defensive rotation)
# ============================================================

def defensive_rotation_diagnostic(r, weight_matrix, rf_daily=None):
    """
    Tests WHY the signal-permutation null is beaten on drawdown metrics but not on Sharpe:
    does momentum avoid the WORST factor in bad months (defensive rotation), improving the
    tail rather than the mean? Weights set at a signal date apply to the following month.
    Returns (summary, per-month frame). 'avoided_worst' = weight on the realised next-month
    worst factor is below equal weight; capture ratios are vs the equal-weight blend.
    """
    r = r.dropna(how="any")
    cols = list(r.columns); n = len(cols)
    Fm = (1.0 + r).groupby(r.index.to_period("M")).prod() - 1.0   # monthly factor returns
    Wp = weight_matrix.copy()
    Wp.index = pd.PeriodIndex(Wp.index, freq="M")
    Wp = Wp[~Wp.index.duplicated(keep="last")].reindex(columns=cols)
    rows = []
    for sigp in Wp.index:
        nxt = sigp + 1
        if nxt not in Fm.index:
            continue
        fnext = Fm.loc[nxt]
        w = Wp.loc[sigp].astype(float)
        worst, best = fnext.idxmin(), fnext.idxmax()
        ew = 1.0 / n
        rows.append({
            "period": nxt.to_timestamp("M"),
            "w_on_worst": float(w[worst]), "underweight_worst": float(ew - w[worst]),
            "avoided_worst": bool(w[worst] < ew - 1e-9),
            "overweight_best": float(w[best] - ew),
            "strat_ret": float(np.nansum(w.values * fnext.values)),
            "ew_ret": float(fnext.mean()),
        })
    ...
    summary = {
        "n_months": int(len(D)),
        "avoid_worst_rate_all": float(D["avoided_worst"].mean()),
        "avoid_worst_rate_down": float(D.loc[down, "avoided_worst"].mean()) if down.any() else np.nan,
        "avoid_worst_rate_up": float(D.loc[~down, "avoided_worst"].mean()) if (~down).any() else np.nan,
        "mean_underweight_worst_bps_mo": float(D["underweight_worst"].mean() * 1e4),
        "downside_capture_vs_ew": float(D.loc[down, "strat_ret"].mean() / D.loc[down, "ew_ret"].mean()) if down.any() else np.nan,
        "upside_capture_vs_ew": float(D.loc[~down, "strat_ret"].mean() / D.loc[~down, "ew_ret"].mean()) if (~down).any() else np.nan,
    }
    return summary, D


# ============================================================
# THESIS EXTENSIONS: long-short academic factor loader (FF / AQR / CSV)
# ============================================================

def load_long_short_factors(source="famafrench", csv_path=None, start="2010-01-01",
                            include=("HML", "SMB", "Mom", "RMW", "CMA"), as_etf_names=False):
    """
    Daily LONG-SHORT factor returns (decimals) for the 'pure factor' contrast. The factor
    momentum of Gupta-Kelly / Ehsani-Linnainmaa lives on these self-financing factor
    returns, not on long-only ETF wrappers; running the SAME signal here localises the
    theory-to-implementation gap to the vehicle.
      source='famafrench' : Kenneth French daily 5 factors + UMD (needs internet).
      source='csv'        : a user CSV (date index, factor columns; %, or decimals).
    Note: these are excess/self-financing returns; the back-test treats each as a
    total-return-like stream (cumprod of 1+f), the standard practitioner proxy.
    """
    if source == "csv":
        if csv_path is None:
            raise ValueError("csv_path required for source='csv'.")
        df = pd.read_csv(csv_path, index_col=0, parse_dates=True).sort_index()
        if df.abs().mean().mean() > 1.0:       # looks like percent
            df = df / 100.0
        return df.astype(float)
    if source == "famafrench":
        from pandas_datareader import data as pdr
        f5 = pdr.DataReader("F-F_Research_Data_5_Factors_2x3_daily", "famafrench", start)[0] / 100.0
        mom = pdr.DataReader("F-F_Momentum_Factor_daily", "famafrench", start)[0] / 100.0
        mom.columns = ["Mom"]
        df = f5.join(mom, how="inner")
        df.columns = [c.strip() for c in df.columns]
        keep = [c for c in include if c in df.columns]
        out = df[keep].copy()
        out.index = pd.to_datetime(out.index)
        if as_etf_names:
            out = out.rename(columns={"HML": "Value", "SMB": "Size", "Mom": "Momentum",
                                      "RMW": "Quality", "CMA": "Investment"})
        return out.sort_index().astype(float)
    raise ValueError("source must be 'famafrench' or 'csv'")
...
def realized_vol_daily(r, lookback=21, ann=252):
    """Trailing annualized realized volatility known at the close of each date."""
    s = pd.Series(r).astype(float)
    min_periods = max(5, int(lookback) // 2)
    return s.rolling(int(lookback), min_periods=min_periods).std(ddof=1) * np.sqrt(ann)


def vol_target_exposure(r, target_vol_ann, lookback=21, max_leverage=1.5,
                        min_exposure=0.0, rebalance_band=0.05, ann=252):
    """Build a causal volatility-target exposure and lag it before returns are earned."""
    s = pd.Series(r).astype(float)
    rv = realized_vol_daily(s, lookback=lookback, ann=ann)
    raw = (float(target_vol_ann) / rv).clip(
        lower=float(min_exposure), upper=float(max_leverage))
    out = pd.Series(index=s.index, dtype=float)
    cur = np.nan
    band = float(rebalance_band)
    for dt, value in raw.items():
        if not np.isfinite(value):
            out.loc[dt] = cur
            continue
        if (not np.isfinite(cur)) or abs(value - cur) > band:
            cur = float(value)
        out.loc[dt] = cur
    return out.shift(1)


def apply_exposure_overlay(r, exposure, rf_daily=None, cost_oneway_bps=0.0):
    """Apply exposure, pay cash at the risk-free rate and charge exposure turnover."""
    s = pd.Series(r).astype(float).dropna()
    exposure = pd.Series(exposure).reindex(s.index).astype(float)
    idx = exposure.dropna().index
    s, exposure = s.loc[idx], exposure.loc[idx]
    rf = _align_rf_to_returns(s, rf_daily=rf_daily).reindex(idx).fillna(0.0)
    cost = float(cost_oneway_bps) / 1e4
    delta = exposure.diff().abs().fillna(0.0)
    out = exposure * s + (1.0 - exposure) * rf - delta * cost
    return out.rename(getattr(s, "name", None))


def vol_target_overlay(r, target_vol_ann, rf_daily=None, lookback=21, max_leverage=1.5,
                       min_exposure=0.0, rebalance_band=0.05,
                       cost_oneway_bps=0.0, ann=252):
    """Build and apply a deterministic volatility-target overlay."""
    exposure = vol_target_exposure(
        r, target_vol_ann, lookback=lookback, max_leverage=max_leverage,
        min_exposure=min_exposure, rebalance_band=rebalance_band, ann=ann)
    out = apply_exposure_overlay(
        r, exposure, rf_daily=rf_daily, cost_oneway_bps=cost_oneway_bps)
    exposure = exposure.reindex(out.index)
    delta = exposure.diff().abs().fillna(0.0)
    return {
        "rets": out,
        "exposure": exposure,
...
def vol_target_grid(strategy_rets, targets_ann, rf_daily=None, lookback=21,
                    max_leverage=1.5, min_exposure=0.0, rebalance_band=0.05,
                    cost_oneway_bps=0.0, ann=252):
    """Apply the same deterministic overlay independently to strategies and benchmarks."""
    frame = (strategy_rets.to_frame() if isinstance(strategy_rets, pd.Series)
             else pd.DataFrame(strategy_rets))
    rets, exposures, rows = {}, {}, []
    metric_names = ("CAGR", "AnnVol", "ShR", "SoR", "MxDD", "Martin")
    for column in frame.columns:
        base = frame[column].dropna()
        base_metrics = perf_metrics(base, rf_daily=rf_daily, ann=ann)
        rows.append({
            "strategy": column, "target": "none (base)", "mean_exposure": 1.0,
            "n_trades": 0, **{k: base_metrics.get(k) for k in metric_names},
        })
        for target in targets_ann:
            result = vol_target_overlay(
                base, target, rf_daily=rf_daily, lookback=lookback,
                max_leverage=max_leverage, min_exposure=min_exposure,
                rebalance_band=rebalance_band, cost_oneway_bps=cost_oneway_bps,
                ann=ann)
            name = f"{column}_VT{target:g}"
            rets[name] = result["rets"].rename(name)
            exposures[name] = result["exposure"]
            metrics = perf_metrics(result["rets"], rf_daily=rf_daily, ann=ann)
            rows.append({
                "strategy": column,
                "target": f"{100 * target:g}%",
                "mean_exposure": result["mean_exposure"],
                "n_trades": result["n_trades"],
                **{k: metrics.get(k) for k in metric_names},
            })
    return {
        "rets": pd.concat(rets.values(), axis=1) if rets else pd.DataFrame(),
        "exposure": pd.DataFrame(exposures),
        "summary": pd.DataFrame(rows),
    }


def lowvol_sleeve_weights(w_mom, sleeve, lowvol_col="LowVol"):
    """Blend a fixed low-volatility sleeve into a monthly weight matrix."""
    weights = pd.DataFrame(w_mom).astype(float).copy()
    if lowvol_col not in weights.columns:
        raise ValueError(
            f"'{lowvol_col}' is not in the weight matrix columns: {list(weights.columns)}")
    sleeve = float(sleeve)
    out = weights * (1.0 - sleeve)
    out[lowvol_col] = out[lowvol_col] + sleeve
    return out.div(out.sum(axis=1), axis=0)


def lowvol_sleeve_backtest(r, signal, sleeve_by_strategy, cost_oneway_daily=None,
                           cost_lookback_days=21, cost_agg="median",
                           charge_initial_trade=True, implementation_lag=1,
                           lowvol_col="LowVol", rf_daily=None, ann=252):
    """Backtest static low-volatility sleeves through the same drift-and-cost engine."""
    builders = {
        "MomTop1_80_20": w_top1_80_20,
        "MomTop2_35_35": w_top2_35_35,
        "EW": w_equal,
    }
    out, rows = {}, []
    metric_names = ("CAGR", "AnnVol", "ShR", "SoR", "MxDD", "Martin")
    for name, sleeve in sleeve_by_strategy.items():
        if name not in builders:
            raise ValueError(f"unknown strategy '{name}'; expected one of {list(builders)}")
        base_weights = builders[name](signal)
        variants = (
            ("", base_weights),
            (f"_LV{int(round(sleeve * 100))}",
             lowvol_sleeve_weights(base_weights, sleeve, lowvol_col)),
        )
        for tag, weights in variants:
            _, net = run_named_strategy(
                r, weights, cost_oneway_daily=cost_oneway_daily,
                cost_lookback_days=cost_lookback_days, cost_agg=cost_agg,
                charge_initial_trade=charge_initial_trade,
                implementation_lag=implementation_lag)
            variant_name = f"{name}{tag}"
            out[variant_name] = net.rename(variant_name)
            metrics = perf_metrics(net, rf_daily=rf_daily, ann=ann)
            rows.append({
                "strategy": variant_name,
                "sleeve": sleeve if tag else 0.0,
                **{k: metrics.get(k) for k in metric_names},
            })
    return {
        "rets": pd.concat(out.values(), axis=1),
